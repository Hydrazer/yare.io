...
if(star_zxq_dist < star_a1c_dist) {
    let base_star = star_zxq;
}
else {
    let base_star = star_a1c;
}
...
enemy_spirits = all_spirits.filter(x => x.hp == 1).filter(x => x.player_id != 'YOUR NAME HERE');
// Gets all spirits that are alive, and belong to you
friendly_spirits = all_spirits.filter(x => x.hp == 1).filter(x => x.player_id == 'YOUR NAME HERE');
...
for(unit of friendly_spirits) {
	...
	if(memory[unit.id] == "base_defender") {
        ...
        var base_invader = spirits[outpost.sight.enemies[0]];
    ...
    if(memory[unit.id] == "base_charging") {
        ...
        if(40000 < find_dist(unit.position, base_star.position)) {
            unit.move(base_star.position);
            continue;
        }
        unit.energize(unit);
    }
