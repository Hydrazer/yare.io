...
// Main Loop
{
  ...
  if (
    find_dist(base.position, star_zxq.position) <
    find_dist(base.position, star_a1c.position)
  ) {
  ...
  enemy_spirits = all_spirits
    .filter((x) => x.hp == 1)
    .filter((x) => x.player_id != "saltAxAtlas");
  ...
  if (current_mode == 0) {
    ...
    for (unit of friendly_spirits) {
      if (harasser_count) {
        ...
        if (
          (outpost.sight.enemies.length > 0 &&
            unit.energy == unit.energy_capacity &&
            outpost.energy > 700) ||
          (outpost.sight.enemies.length > 0 &&
        ...
        if (
          (base.sight.enemies.length > 0 &&
            unit.energy == unit.energy_capacity) ||
          (base.sight.enemies.length > 0 && memory[unit.id] == "base_defender")
        ) {
          memory[unit.id] = "base_defender";
        } else {
      ...
      if (memory[unit.id] == "harasser") {
        ...
        if (number_enemy_units_harasser > 0) {
          ...
          let closest_enemy_harasser = get_closest_enemy(
            unit.sight.enemies,
            unit.position
          );
          let distance_to_enemy = find_dist(
            unit.position,
            closest_enemy_harasser.position
          );
          if (
            number_enemy_units_harasser >= 3 &&
            find_dist(unit.position, outpost.position) > 30000
          ) {
            unit.move(outpost.position);
          } else if (
            distance_to_enemy > 40000 &&
            unit.energy > closest_enemy_harasser.energy
          ) {
            unit.move(closest_enemy_harasser.position);
          } else if (
            distance_to_enemy > 40000 &&
            unit.energy < harass_invader.energy
          ) {
        ...
        let closest_enemy_outpost_defender = get_closest_enemy(
          outpost.sight.enemies,
          unit.position
        );
        let distance_to_enemy_at_outpost = find_dist(
          unit.position,
          closest_enemy_outpost_defender.position
        );
        ...
        let closest_enemy_base = get_closest_enemy(
          base.sight.enemies,
          unit.position
        );
        let distance_to_enemy_at_base = find_dist(
          unit.position,
          closest_enemy_base.position
        );
